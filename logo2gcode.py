#----------------------------------------------------------------------
# based on https://github.com/rcpedersen/snap-to-gcode
# 
...
if __name__ == '__main__':
    if len(sys.argv) != 7:
        print('Usage: python logo2gcode logo_file gcode_file up_height down_height arc_min_len ratio')
        sys.exit(1)

    logo_fname = sys.argv[1]        
    gcode_fname = sys.argv[2]
    f1 = float(sys.argv[3])
    f2 = float(sys.argv[4])
    f3 = float(sys.argv[5])
    f4 = float(sys.argv[6])
    print('logo_filename=%s gcode_filename:%s, up_height=%f, down_height=%f, arc_min_len=%f, ratio=%f' %(logo_fname,gcode_fname,f1,f2,f3,f4))
    
    f = open(gcode_fname,'w')
    t = logo2gcode(f,f1,f2,f3,f4)
    t.load_file(logo_fname)
    #print('lines:',t.lines)
    t.translate(t.lines)
